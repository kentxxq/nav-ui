  ...
  <Dialog v-model:visible="visible" modal header="新增书签" class="w-100" :dismissableMask="true">
    <Form v-slot="$form" :initialValues :resolver="resolver" @submit="onFormSubmit" :validateOnValueUpdate="true"
      ...
      <div class="flex flex-col gap-1">
        <InputText :inputProps="{ autocomplete: 'true' }" name="url" placeholder="站点链接" toggleMask :feedback="false"
          fluid />
        <Message v-if="$form.url?.invalid" severity="error" size="small" variant="simple">
          {{ $form.url.error?.message }}</Message>
      </div>
      <div class="flex flex-col gap-1">
        <InputText :inputProps="{ autocomplete: 'true' }" name="title" placeholder="标题" toggleMask :feedback="false"
          fluid />
        <Message v-if="$form.title?.invalid" severity="error" size="small" variant="simple">
          {{ $form.title.error?.message }}</Message>
      </div>
      <div class="flex flex-col gap-1">
        <InputText :inputProps="{ autocomplete: 'true' }" name="iconUrl" placeholder="图标链接" toggleMask :feedback="false"
          fluid />
        <Message v-if="$form.iconUrl?.invalid" severity="error" size="small" variant="simple">
          {{ $form.iconUrl.error?.message }}</Message>
      </div>
      <div class="flex flex-col gap-1">
        <InputText :inputProps="{ autocomplete: 'true' }" name="description" placeholder="说明" toggleMask
...
const initialValues = reactive<CreateBookmarkRO>({
  url: '',
  title: '',
  iconUrl: '',
  description: null,
...
const onFormSubmit = async (e: FormSubmitEvent) => {
  if (e.valid) {
    const bookmark = await bookmarkCreateBookmark({ url: e.values.url, title: e.values.title, iconUrl: e.values.iconUrl, description: e.values.description })
